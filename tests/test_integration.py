...
class TestConfigPropagation:
    """Test that config values propagate through the workflow"""

    def test_custom_timeout_is_used(self):
        """Custom timeout should be passed to HTTP client"""
        mock_client = Mock()
        mock_response = Mock()
        mock_response.status_code = 200
        ...
        config = Config(
            {
                "scraper": {"headers": {"user_agent": "Test"}},
                "api": {"timeouts": {"external_url": 99}},  # Custom timeout
            }
        )

        fetch_external_details(
            external_url="https://test.com", http_client=mock_client, config_obj=config
        )
    ...
    def test_custom_user_agent_is_used(self):
        """Custom user agent should be included in headers"""
        mock_client = Mock()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = "<main>Content</main>"
        ...
        config = Config(
            {
                "scraper": {"headers": {"user_agent": "CustomBot/2.0"}},
                "api": {"timeouts": {"external_url": 15}},
            }
        )
