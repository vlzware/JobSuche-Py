...
class TestAPIKeyValidation:
    """Test API key requirements and environment handling"""

    def test_api_key_required_for_classification(self):
        """API key required when classification is enabled"""
        # Setup - ensure no API key in environment
    ...
    @patch("main.SearchSession")
    @patch("main.JobGatherer")
    def test_api_key_not_required_with_no_classification(self, mock_gatherer, mock_session):
        """API key not required when --no-classification is used"""
        # Setup
        env_backup = os.environ.get("OPENROUTER_API_KEY")
        if "OPENROUTER_API_KEY" in os.environ:
            del os.environ["OPENROUTER_API_KEY"]
        ...
        mock_gatherer_instance.gather.return_value = (
            [],
            [],
            {"total_found": 0, "successfully_extracted": 0},
        )
        ...
        test_args = ["main.py", "--was", "Developer", "--wo", "Berlin", "--no-classification"]

        try:
            with patch.object(sys, "argv", test_args):
                # Should not raise SystemExit for missing API key, but will exit(0) for no jobs
    ...
    def test_api_key_from_environment_variable(
        ...
        """Should accept API key from OPENROUTER_API_KEY environment variable"""
        # Setup
        ...
        with patch.object(sys, "argv", test_args):
            # Will exit(0) due to no jobs, but should not fail on API key validation
            with pytest.raises(SystemExit) as exc_info:
                main.main()
            assert exc_info.value.code == 0
    ...
    @patch("main.JobGatherer")
    @patch("main.MultiCategoryWorkflow")
    def test_api_key_from_cli_argument(
        self, mock_workflow, mock_gatherer, mock_llm, mock_profile, mock_session
    ):
        """--api-key argument should work"""
        # Setup - remove env var to ensure CLI arg is tested
        env_backup = os.environ.get("OPENROUTER_API_KEY")
        if "OPENROUTER_API_KEY" in os.environ:
            del os.environ["OPENROUTER_API_KEY"]
        ...
        test_args = ["main.py", "--was", "Developer", "--wo", "Berlin", "--api-key", "cli-test-key"]
