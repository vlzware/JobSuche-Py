...
class TestHttpClientMocking:
    """Test that HttpClient can be easily mocked in tests"""

    def test_mock_http_client_for_testing(self):
        """HttpClient should be easy to mock for dependency injection"""
        ...
        # Use in a function that accepts http_client parameter
        def fetch_page(url, http_client):
            response = http_client.get(url, timeout=10)
            return response.text

        # Test with mocked client
        result = fetch_page("https://test.com", mock_client)

        assert result == "<html>Test content</html>"
        mock_client.get.assert_called_once_with("https://test.com", timeout=10)

    def test_multiple_mock_responses(self):
        """Should support multiple different mock responses"""
        mock_client = Mock()

        # First call returns 200
        response_1 = Mock()
        response_1.status_code = 200
        response_1.text = "Success"

        # Second call returns 404
        response_2 = Mock()
        response_2.status_code = 404

        mock_client.get.side_effect = [response_1, response_2]

        # Make two calls
        result1 = mock_client.get("https://test.com/1")
        result2 = mock_client.get("https://test.com/2")
