...
class TestScraperEdgeCases:
    ...
    @staticmethod
    def _make_long_content(main_text):
        """Helper to ensure content exceeds MIN_VALID_TEXT_LENGTH (1000 chars)"""
        filler = " We are looking for talented developers who are passionate about technology and want to work on challenging projects in a collaborative environment. Our team values innovation, continuous learning, and work-life balance. We offer competitive salaries, comprehensive benefits, flexible working arrangements, and opportunities for professional growth and development."
        return main_text + filler * 3  # Adds ~750 chars

    def test_parse_external_page_with_unicode_characters(self, test_config):
        """Should handle German umlauts and special characters"""
        main_content = "Software-Entwickler für München. Wir suchen einen erfahrenen Entwickler in München mit umfassender Erfahrung in C++. Gehalt: €50.000 - €70.000 pro Jahr. Über uns: Die Große Firma GmbH & Co. KG ist ein führendes Technologieunternehmen mit Sitz in München."

        html = f"""<html><body><main>{self._make_long_content(main_content)}</main></body></html>"""
    ...
    def test_parse_external_page_with_emojis(self, test_config):
        """Should handle emojis in job descriptions"""
        main_content = "🚀 Exciting Startup Position 🎯 Join our amazing tech team! 💻 We're looking for talented developers who want to work from anywhere in the world 🌍"
        ...
        assert result["success"]
        assert "Exciting" in result["text"] or "team" in result["text"]

    def test_parse_external_page_with_malformed_html(self, test_config):
        """Should handle malformed/unclosed HTML tags gracefully"""
        main_content = "Job Title for Software Engineer Position - We are looking for an experienced software engineer."
        html = f"""<html><body><main>{self._make_long_content(main_content)}</main></body></html>"""
        config = Config(test_config)
        result = parse_external_page(html, "https://example.com", config)
        assert result["success"]
        assert "Software" in result["text"]

    def test_parse_external_page_with_broken_html_entities(self, test_config):
        """Should handle broken HTML entities"""
        main_content = "Senior Developer Position - Salary &euro; 50000 to &euro; 80000. Location M&uuml;nchen. Company A&amp;B GmbH."
        html = f"""<html><body><main>{self._make_long_content(main_content)}</main></body></html>"""
        config = Config(test_config)
        result = parse_external_page(html, "https://example.com", config)
        assert result["success"]
        assert "50000" in result["text"] or "Developer" in result["text"]

    def test_parse_external_page_with_very_large_html(self, test_config):
        """Should handle extremely large HTML documents"""
        # Create a large HTML document (~1MB)
        large_content = "<p>Job description paragraph. " * 10000  # ~300KB
        html = f"""
        <html><body>
        <main>
            <h1>Large Job Posting</h1>
            {large_content}
        </main>
        </body></html>
        """
        ...
        assert result["success"]
        assert "Large Job Posting" in result["text"]
        assert len(result["text"]) > 100000  # Should have substantial content

    def test_parse_external_page_with_empty_html(self, test_config):
        """Should handle completely empty HTML"""
        html = ""
        ...
        assert not result["success"] or len(result.get("text", "")) == 0

    def test_parse_external_page_with_only_whitespace(self, test_config):
        ...
        # Should detect as empty/short content
        assert not result["success"] or result.get("warning") in (
            "SHORT_CONTENT",
            "LOW_QUALITY",
        )

    def test_parse_external_page_with_nested_tags(self, test_config):
        """Should handle deeply nested HTML structures"""
        main_content = "We are seeking a talented software engineer to join our team and work on cutting-edge technology projects."
        html = f"""<html><body><main><div><div><div>{self._make_long_content(main_content)}</div></div></div></main></body></html>"""
        config = Config(test_config)
        result = parse_external_page(html, "https://example.com", config)
        assert result["success"]
        assert "software" in result["text"]
    ...
    def test_clean_text_handles_multiple_spaces(self):
        """Should clean multiple consecutive spaces"""
        text = "Job    description    with    many    spaces"
        result = clean_text(text)
    ...
    def test_parse_arbeitsagentur_with_mixed_encoding(self, test_config):
        """Should handle mixed character encodings"""
        html = """
        <html><body>
        <jb-steadetail-beschreibung>
            <p>Stellenbeschreibung für München</p>
            <p>Bewerbung über unsere Website möglich</p>
        </jb-steadetail-beschreibung>
        </body></html>
        """
        ...
        assert result["success"]
        assert "München" in result["text"]
        assert "möglich" in result["text"]

    def test_fetch_external_details_with_connection_timeout(self, test_config):
    ...
    def test_fetch_external_details_with_various_http_errors(self, test_config):
        """Should handle various HTTP error codes"""
        config = Config(test_config)

        error_codes = [500, 502, 503, 504, 403, 404]
    ...
    def test_parse_external_page_with_special_html_chars(self, test_config):
        """Should handle special HTML characters"""
        main_content = "Senior C++ Developer Position - Salary > €50k and < €100k. Skills: C++ & Python. Benefits: Great company."
        html = f"""<html><body><main>{self._make_long_content(main_content)}</main></body></html>"""
        config = Config(test_config)
        result = parse_external_page(html, "https://example.com", config)
        assert result["success"]
        assert "50k" in result["text"] or "Developer" in result["text"]

    def test_extract_domain_with_complex_urls(self):
        """Should extract netloc (domain with port if present) from complex URLs"""
        test_cases = [
            (
                "https://jobs.example.com:8080/path?param=value",
                "jobs.example.com:8080",
            ),  # Includes port
            ("http://www.example.com/", "www.example.com"),
            ("https://sub.domain.example.com/job", "sub.domain.example.com"),
    ...
    def test_extract_domain_with_invalid_urls(self):
        ...
        for url in invalid_urls:
            if url is None:
                continue
            result = extract_domain(url)
            # Should return empty string or the input itself
